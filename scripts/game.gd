...
const DOOR_GAMEPLAY_ENABLED := false
...
@onready var tower_connections: Node2D = $TowerConnections
...
@onready var expand_safe_zone_task: Label = $GameInterface/PlayerPanel/Margin/VBox/ExpandSafeZoneTask
@onready var find_upgrade_stations_task: Label = $GameInterface/PlayerPanel/Margin/VBox/FindUpgradeStationsTask
...
@onready var story_screen: Control = $StoryOverlay/StoryScreen
...
var _build_mode := BuildMode.TURRET
...
func _ready() -> void:
	...
	_configure_structure_movement_collision(
		player,
		PLAYER_STRUCTURE_COLLISION_ENABLED
	)
	player.set_collision_mask_value(3, PLAYER_ENEMY_COLLISION_ENABLED)
...
func _process(delta: float) -> void:
	...
	_update_structure_attack_status()
...
func information_marker_cells() -> Array[Vector2i]:
	return _information_marker_cells.duplicate()


func has_information_marker_at(cell: Vector2i) -> bool:
	return _information_marker_cells.has(cell)


func add_information_marker(cell: Vector2i) -> void:
	if not _information_marker_cells.has(cell):
		_information_marker_cells.append(cell)
	queue_redraw()


func remove_information_marker(cell: Vector2i) -> void:
	_information_marker_cells.erase(cell)
	queue_redraw()
...
func _update_visibility() -> void:
	...
	for turret: Turret in _turrets:
		var turret_cell := maze.world_to_cell(turret.position)
		turret.update_visibility(
			maze.is_cell_visible(turret_cell),
			maze.is_cell_explored(turret_cell)
		)
	for tower: Tower in _towers:
		var tower_cell := maze.world_to_cell(tower.position)
		tower.update_visibility(
			maze.is_cell_visible(tower_cell),
			maze.is_cell_explored(tower_cell)
		)
...
func _update_structure_attack_status() -> void:
	var tower_attacked := false
	var turret_attacked := false
	for tower: Tower in _towers:
		if is_structure_under_attack(tower):
			tower_attacked = true
			break
	for turret: Turret in _turrets:
		if is_structure_under_attack(turret):
			turret_attacked = true
			break
...
func notify_structure_attacked(structure: Node2D) -> void:
	if not is_instance_valid(structure):
		return
	_structure_attack_highlight_until[
		structure.get_instance_id()
...
func _update_player_panel() -> void:
	...
	if turret_count != _displayed_turret_inventory:
		...
		turrets_value.text = tr("Турели: %d") % turret_count
	var tower_count := player.tower_inventory_count()
	if tower_count != _displayed_tower_inventory:
		_displayed_tower_inventory = tower_count
...
func save_game() -> bool:
	var save_data := {
		...
		"turret_health_upgrade_level": player.turret_health_upgrade_level,
		...
		"information_marker_cells": _information_marker_cells.map(
			func(cell: Vector2i): return [cell.x, cell.y]
		),
		...
		"towers": _towers.map(func(tower: Node): return tower.save_data()),
...
func _restore_game(save_data: Dictionary) -> void:
	...
	var saved_information_marker_cells: Array = save_data.get(
		"information_marker_cells",
		[]
	)
	for saved_cell: Variant in saved_information_marker_cells:
		if saved_cell is Array and saved_cell.size() == 2:
			_information_marker_cells.append(Vector2i(
				int(saved_cell[0]),
				int(saved_cell[1])
			))
	...
	if save_data.has("doors"):
		...
		_create_generated_doors()
	for turret_data in save_data.get("turrets", []):
		if turret_data is Dictionary:
...
func turret_enemy_level_summary() -> String:
	...
	for level in range(1, ENEMY_LEVEL_COUNT + 1):
		...
		lines.append(tr(
			...
			_average_hits_to_kill(
				enemy_health,
				player.turret_damage_min(),
				player.turret_damage_max()
			),
			enemy_damage_min,
			enemy_damage_max,
			_average_hits_to_kill(
				player.turret_max_health(),
				enemy_damage_min,
				enemy_damage_max
			),
		])
...
func _create_enemy(
	...
	_configure_structure_movement_collision(
		enemy,
		ENEMY_STRUCTURE_COLLISION_ENABLED
	)
	enemy.set_collision_mask_value(2, PLAYER_ENEMY_COLLISION_ENABLED)
...
func _configure_structure_movement_collision(
	body: CollisionObject2D,
	enabled: bool
) -> void:
	# Layers 4 and 5 contain turrets and towers. Their hit shapes stay active
	# so projectiles can still damage them when movement collision is disabled.
	body.set_collision_mask_value(4, enabled)
	body.set_collision_mask_value(5, enabled)
...
func _update_build_action(delta: float) -> void:
	...
	match completed_action:
		...
		BuildActionType.PLACE_TOWER:
			_finish_place_tower()
		BuildActionType.REMOVE_TOWER:
			_finish_remove_tower()
	_clear_build_action()
...
func _build_action_label(action_type: BuildActionType) -> String:
	return (
		...
		if action_type == BuildActionType.REMOVE_DOOR \
				...
				or action_type == BuildActionType.REMOVE_TOWER
		else tr("установка")
	)
...
func _start_turret_action_at(target_position: Vector2) -> void:
	...
	placement_direction = placement_direction.normalized()
...
func _turret_at(cell: Vector2i) -> Turret:
	for turret: Turret in _turrets:
		if turret.cell == cell:
			return turret
	return null
...
func _create_turret_from_save(saved_data: Dictionary) -> void:
	...
	if saved_cell.size() != 2:
		return
	var cell := Vector2i(int(saved_cell[0]), int(saved_cell[1]))
	if not maze.is_cell_walkable(cell) or _turret_at(cell) != null:
		return
	var saved_position: Array = saved_data.get("position", [])
...
func _start_tower_action_at(target_position: Vector2) -> void:
	var placement_direction := player.position.direction_to(target_position)
	if placement_direction.is_zero_approx():
		placement_direction = player.facing_direction()
	placement_direction = placement_direction.normalized()
	...
	if existing_tower != null:
		...
		_start_build_action(
			BuildActionType.REMOVE_TOWER,
			target_cell,
			existing_tower.position,
			placement_direction,
			false
		)
		return
	if player.tower_inventory_count() <= 0 \
			or not maze.is_cell_walkable(target_cell) \
...
func _finish_place_tower() -> void:
	_place_tower(_build_action_cell, _build_action_position)


func _place_tower(target_cell: Vector2i, world_position: Vector2) -> void:
	if player.tower_inventory_count() <= 0 \
			or not maze.is_cell_walkable(target_cell) \
			or _turret_at(target_cell) != null \
			or _tower_at(target_cell) != null \
			or _has_door_at(target_cell):
		return
	var tower_data := player.take_tower_from_inventory()
	if tower_data.is_empty():
		return
	_create_tower(
		target_cell,
		world_position,
		int(tower_data.get("health", Player.TOWER_MAX_HEALTH))
	)
...
func _finish_remove_tower() -> void:
	_remove_tower(_build_action_cell)


func _remove_tower(target_cell: Vector2i) -> void:
	var existing_tower := _tower_at(target_cell)
...
func _tower_at(cell: Vector2i) -> Tower:
	for tower: Tower in _towers:
		if tower.cell == cell:
			return tower
	return null
...
func _create_tower(
	cell: Vector2i,
	world_position: Vector2,
	health_value: int = Player.TOWER_MAX_HEALTH
) -> Tower:
	var tower: Tower = TOWER_SCENE.instantiate()
	tower.setup(self, cell, world_position, health_value)
	towers.add_child(tower)
	_towers.append(tower)
	return tower


func _create_tower_from_save(saved_data: Dictionary) -> void:
	var saved_cell: Array = saved_data.get("cell", [])
	if saved_cell.size() != 2:
		return
	var cell := Vector2i(int(saved_cell[0]), int(saved_cell[1]))
	if not maze.is_cell_walkable(cell) or _tower_at(cell) != null \
			or _turret_at(cell) != null:
		return
	var saved_position: Array = saved_data.get("position", [])
	var world_position := maze.cell_to_world(cell)
	if saved_position.size() == 2:
		world_position = Vector2(
			float(saved_position[0]),
			float(saved_position[1])
	...
	_create_tower(
		cell,
		world_position,
		int(saved_data.get("health", Player.TOWER_MAX_HEALTH))
	)
...
func _refresh_safe_zone(recompute_base_zone: bool = true) -> void:
	var door_cells: Array[Vector2i] = []
	for door: Door in _doors:
		door_cells.append(door.cell)

	if recompute_base_zone:
...
func _tower_connection_is_clear(from: Vector2, to: Vector2) -> bool:
	var from_cell := maze.world_to_cell(from)
	var to_cell := maze.world_to_cell(to)
	var from_index := from_cell.y * Maze.COLUMNS + from_cell.x
	var to_index := to_cell.y * Maze.COLUMNS + to_cell.x
	var lower_index := mini(from_index, to_index)
	var upper_index := maxi(from_index, to_index)
...
func _interact_with_turret() -> bool:
	...
	var closest_turret: Turret
	var closest_distance := INF
	for turret: Turret in _turrets:
		var direction_to_turret := turret.position - player.position
		var distance := direction_to_turret.length()
		if distance > Maze.CELL_SIZE * 1.5:
			continue
		if not direction_to_turret.is_zero_approx() \
				and facing.dot(direction_to_turret.normalized()) < 0.25:
			continue
		if distance < closest_distance:
	...
	if closest_turret == null:
		return false
	turret_menu.open(closest_turret)
	return true
...
func _interact_with_tower() -> bool:
	...
	var closest_distance := INF
	for tower: Tower in _towers:
		var direction_to_tower := tower.position - player.position
		var distance := direction_to_tower.length()
		if distance > Maze.CELL_SIZE * 1.5:
			continue
		if not direction_to_tower.is_zero_approx() \
				and facing.dot(direction_to_tower.normalized()) < 0.25:
			continue
		if distance < closest_distance:
			closest_tower = tower
			closest_distance = distance
...
func _show_station_story(station_id: int) -> void:
	_pending_story_station_id = station_id
	player.controls_enabled = false
	story_screen.open(
		tr("Заметка %d") % station_id,
		LoreText.note_text(station_id)
	)
...
func _on_story_continued() -> void:
	if _final_story_pending:
		...
		return
	player.controls_enabled = true
	if _pending_story_station_id > 0:
		var station_id := _pending_story_station_id
		_pending_story_station_id = 0
		var show_instructions := _show_instructions_after_story
		_show_instructions_after_story = false
		if show_instructions:
			_station_instructions_seen = true
		station_menu.open(show_instructions, station_id)
	else:
...
func upgrade_turret_health() -> bool:
	...
	if not player.upgrade_turret_health():
		return false
	var new_max := player.turret_max_health()
	for turret: Turret in _turrets:
		turret.apply_upgraded_maximums(
			new_max,
			player.turret_max_ammo(),
...
func upgrade_turret_ammo() -> bool:
	...
	if not player.upgrade_turret_ammo():
		return false
	var new_max := player.turret_max_ammo()
	for turret: Turret in _turrets:
		turret.apply_upgraded_maximums(
			player.turret_max_health(),
			new_max,
...
func upgrade_tower_health() -> bool:
	...
	if not player.upgrade_tower_health():
		return false
	var new_max := player.tower_max_health()
	for tower: Tower in _towers:
		tower.apply_upgraded_maximum(new_max, new_max > previous_max)
	_update_player_panel()
	return true
...
func spawn_turret_bullet(
	...
	bullet.setup(
		...
		direction,
		maze,
		_rng.randi_range(player.turret_damage_min(), player.turret_damage_max()),
		true
	)
	bullets.add_child(bullet)
	_alert_enemies_to_shot(
...
func visible_structure_for_enemy(
	enemy: Enemy,
	range_cells: float,
	facing_direction: Vector2,
	half_angle: float
